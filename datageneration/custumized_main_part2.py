...
if __name__ == '__main__':
    ...
    # tmp_path = join(tmp_path, 'run%d_%d' % (idx, ishape))
    res_paths = {k:join(tmp_path, '%05d_%s'%(idx, k)) for k in output_types if output_types[k]}

    data = cmu_parms[name]
    nframes = len(data['poses'][::stepsize])
    output_path = join(output_path, 'run{}_{}'.format(idx, ishape))
    
    # .mat files
    # matfile_normal = join(output_path, name.replace(" ", "") + "_c%04d_normal.mat" % (ishape ))
    # matfile_gtflow = join(output_path, name.replace(" ", "") + "_c%04d_gtflow.mat" % (ishape))
    # matfile_depth = join(output_path, name.replace(" ", "") + "_c%04d_depth.mat" % (ishape))
    matfile_segm = join(output_path, "c%04d_segm.mat" % (ishape))
    dict_normal = {}
    dict_gtflow = {}
    dict_depth = {}
    dict_segm = {}
